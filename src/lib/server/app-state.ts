...
export const ADMIN_SESSION_COOKIE = "newstart-admin-ok";
export const TEAM_SESSION_COOKIE = "newstart-team-id";
...
export function hasAdminCookie(cookies: CookieReader) {
  return cookies.get(ADMIN_SESSION_COOKIE)?.value === "1";
}

export function getTeamCookie(cookies: CookieReader) {
  return cookies.get(TEAM_SESSION_COOKIE)?.value ?? null;
}
...
export async function readAppState(options: { includeLoginCodes?: boolean } = {}): Promise<AppState> {
  ...
  return {
    ...
    teams: ((teams.data ?? []) as DbTeam[]).map((row): Team => ({
      ...
      name: row.name,
      loginCode: options.includeLoginCodes ? row.login_code : "",
...
export async function writeAppState(state: AppState) {
  ...
  const results = await Promise.all([
    ...
    state.easterEggClaims.length
      ? supabase.from("team_easter_egg_claims").upsert(
      ...
      : Promise.resolve({ error: null }),
    state.adminAwards.length
      ? supabase.from("admin_awards").upsert(
